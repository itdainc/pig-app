...
try:
  ...
  def allocate_pigs_data(file, specs):
    return pd.DataFrame(), {}
...
if check_password():
    ...
    for col in df.columns:
      if col in ['우선순위', '목표두수']:
        config[col] = st.column_config.Column(
            col, alignment='center', width='small'
        )
      elif col == '비고':
        config[col] = st.column_config.Column(
            col, alignment='center', width='large'
        )
      elif col in ['출하농가', '이력번호']:
        config[col] = st.column_config.Column(
            col, alignment='center', width='medium'
        )
      else:
  ...
  if st.session_state.main_menu == '배정':
    ...
    with tab1:
      ...
      if uploaded_grade:
        try:
          df_raw, w_col_name, f_col_name = load_excel_by_coords(
              uploaded_grade
          )
        ...
        try:
          if run_first_allocation:
            ...
            if display_summary is not None and not display_summary.empty:
              ...
              c2.metric(
                  '일반 거래처 배정 수량',
                  f"{len(pigs[~pigs['배정거래처'].str.contains('잇다', na=False)])} 두",
              )
              c3.metric(
                  '잇다 잔여 할당 수량',
                  f"{len(pigs[pigs['배정거래처'].str.contains('잇다', na=False)])} 두",
              ...
              with col_main:
                ...
                summary = (
                    pigs.groupby(['배정거래처', '성별'])
                    .size()
                    .unstack(fill_value=0)
                )
                if '거세' not in summary.columns:
                  summary['거세'] = 0
                if '암' not in summary.columns:
                  summary['암'] = 0
      ...
      with st.expander(
          '📌 거래처별 목표두수 수기 조정 표 (클릭하여 열기)', expanded=True
      ):
        st.info(
            '💡 목표 두수에 변동이 있으면 아래 표에서 수량을 직접 수정하거나 새로운'
            ' 거래처를 추가하세요.'
        )
        edited_target_df = st.data_editor(
            ...
            key='target_editor',
            column_config={
                '거래처명': st.column_config.Column(
                    '거래처명', alignment='center'
                ),
                '목표두수': st.column_config.Column(
                    '목표두수', alignment='center'
                ),
            },
        )
        if st.button(
    ...
    with tab2:
      ...
      if (
          ...
          and 'specs_dict' in st.session_state
      ):
        pigs_all = st.session_state['allocated_pigs']
        specs_dict = st.session_state['specs_dict']

        all_assigned = [
            c for c in pigs_all['배정거래처'].unique() if '잇다' not in c
        ]
        main_company_map = {}
        ...
        if company_list:
          if (
              ...
              or st.session_state.selected_company not in company_list
          ):
          ...
          for idx, comp in enumerate(company_list):
            ...
            if cols[col_idx].button(
                f'📌 {comp}' if is_selected else comp,
                key=f'btn_comp_{comp}',
          ...
          for sub_comp in sub_companies:
            ...
            for idx, row in comp_df.iterrows():
              ...
              if row['중량'] < spec['w_min']:
                diffs.append(
                    f"중량미달({row['중량']}kg < {spec['w_min']}kg)"
                )
              elif row['중량'] > spec['w_max']:
                diffs.append(
                    f"중량초과({row['중량']}kg > {spec['w_max']}kg)"
                )
              if row['등지방'] < spec['f_min']:
                diffs.append(
                    f"등지방미달({row['등지방']}mm < {spec['f_min']}mm)"
                )
              elif row['등지방'] > spec['f_max']:
                diffs.append(
                    f"등지방초과({row['등지방']}mm > {spec['f_max']}mm)"
                )
              if (
                  spec['grades']
                  and str(row['등급']).strip() not in spec['grades']
              ):
                diffs.append(f"등급불일치({row['등급']})")
              if spec['배제농가'] and any(
                  farm in str(row['출하농가']) for farm in spec['배제농가']
              ):
            ...
            m4.metric(
                '평균 중량',
                f"{comp_df['중량'].mean():.1f} kg"
                if not comp_df.empty
                else '0 kg',
            )
            m5.metric(
                '평균 등지방',
                f"{comp_df['등지방'].mean():.1f} mm"
                if not comp_df.empty
                else '0 mm',
            )
            ...
            with pd.ExcelWriter(output_comp, engine='openpyxl') as writer:
              comp_df.to_excel(writer, sheet_name=sub_comp)

            st.download_button(
                label=f'📥 [{sub_comp}] 배정 명단 엑셀 다운로드',
                data=output_comp.getvalue(),
                file_name=f'{sub_comp}_배정명단.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            )
    ...
    with tab3:
      ...
      if 'allocated_pigs' in st.session_state:
        pigs_all = st.session_state['allocated_pigs']
        jn_pigs = pigs_all[
            pigs_all['배정거래처'].str.contains('잇다', na=False)
        ].copy()
        ...
        if not jn_pigs.empty:
          for ita_type in sorted(jn_pigs['배정거래처'].unique()):
            ...
            with pd.ExcelWriter(output_jn, engine='openpyxl') as writer:
              jn_export.to_excel(writer, sheet_name=ita_type, index=False)
            today_str = datetime.now().strftime('%m%d')

            st.download_button(
                label=(
                    f'📥 [{ita_type}] 전달용 엑셀 다운로드 ({today_str}'
                    f' {ita_type}.xlsx)'
                ),
                data=output_jn.getvalue(),
                file_name=f'{today_str}_{ita_type}.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            )
            calc_jn_height = (len(jn_export) + 1) * 35 + 10
            st.dataframe(
                jn_export,
                height=calc_jn_height,
                use_container_width=True,
  ...
  # ==============================================================================
    ...
    if 'allocated_pigs' in st.session_state:
      ...
      pigs_all[['농가_명', '사료사_명']] = pigs_all['출하농가'].apply(
      ...
      for (farm_name, feed_name), group in farm_groups:
        ...
        rows.append({
            ...
            '암': f"{len(group[group['성별'] == '암']):,}",
            '1+': f'{cnt_1plus:,}',
            '1+ 중량': (
                f"{int(round(group[group['등급'] == '1+']['중량'].sum())):,}"
            ),
            '1': f'{cnt_1:,}',
            '1 중량': (
                f"{int(round(group[group['등급'] == '1']['중량'].sum())):,}"
            ...
            '2': f'{cnt_2:,}',
            '2 중량': (
                f"{int(round(group[group['등급'] == '2']['중량'].sum())):,}"
            ),
            '1+,1 비율': (
      ...
      sum_row = pd.DataFrame([{
          ...
          '스펙비율': (
              f"{(len(pigs_all[(pigs_all['중량'] >= 86) & (pigs_all['중량'] <= 96) & (pigs_all['등지방'] >= 19) & (pigs_all['등지방'] <= 23)]) / total_head * 100):.1f}%"
              if total_head > 0
              else '0.0%'
          ),
          '암': f"{len(pigs_all[pigs_all['성별'] == '암']):,}",
          '1+': f"{len(pigs_all[pigs_all['등급'] == '1+']):,}",
          '1+ 중량': (
              f"{int(round(pigs_all[pigs_all['등급'] == '1+']['중량'].sum())):,}"
          ),
          '1': f"{len(pigs_all[pigs_all['등급'] == '1']):,}",
          '1 중량': (
              f"{int(round(pigs_all[pigs_all['등급'] == '1']['중량'].sum())):,}"
          ),
          '2': f"{len(pigs_all[pigs_all['등급'] == '2']):,}",
          '2 중량': (
              f"{int(round(pigs_all[pigs_all['등급'] == '2']['중량'].sum())):,}"
          ),
          '1+,1 비율': (
              f"{((len(pigs_all[pigs_all['등급'] == '1+']) + len(pigs_all[pigs_all['등급'] == '1'])) / total_head * 100):.2f}%"
              if total_head > 0
              else '0.00%'
          ),
      }])

      final_analysis_df = pd.concat(
          [analysis_df, sum_row], ignore_index=True
      )
